<template>
	<view>
		<view class="cu-progress-main" :style="{'border-radius':bgBR,'width': progressMainW,'height':progressMainH,'background-color': backgroundColor}">
			<text v-if="showInfoStatus && infoAlign == 'left'" class="cu-showInfo" :style="{'color':infoColor,'font-size':infoS,width:infoW}">{{showValue}}{{infoEndText}}</text>
			<view class="cu-progress-bar" :style="{'left':areaLeft, 'position':pgPn, 'bottom':progressBt,'background':pgbg,'margin-left':progressML, 'margin-bottom':progressMB,'width': progressW,'height':progressH,'border-radius':pgBR,'background-color':noActiveColor}"></view>
			<movable-area class="cu-area" :style="{'left':areaLeft, 'flex-direction':flexDirection,'width':areaW,height:areaH}" @touchcancel="touchCancel" @touchstart="areaTouchStart"
			...
			<text v-if="showInfoStatus && infoAlign == 'right'" class="cu-showInfo" :style="{'right':infoRt, 'color':infoColor,'font-size':infoS,width:infoW}">{{showValue}}{{infoEndText}}</text>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			getStepInfo() {
				// return step, decimal 位数
				let step = Number(this.step)
				if (step <= 0 || !step){
					return [1, 0]
				}
				else{
					let steps = this.step.toString().split('.')
					if (steps.length == 1){
						return [step,0]
					}
					else {
			...
			textLength(t) {
				...
				let l = (t.length - subl + this.getStepInfo()[1]) / 3 * 2 + subl
			...
			handleMove(x_) {
				...
				if (this.direction == 'vertical'){
					...
					let valueE = value * 10 ** stepInfo[1]
