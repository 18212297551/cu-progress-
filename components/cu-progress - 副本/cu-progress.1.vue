<template>
	<view>
		<view class="cu-progress-main" :style="{'flex-direction':flexDirection, 'border-radius':bgBR,'width': progressMainW,'height':progressMainH,'background-color': backgroundColor}">
		...
		methods: {
			...
			handleMove(x_) {
				...
				if (this.direction == 'vertical'){
					...
					this.showValue = sv
					this.handleY = x
					console.log('x:' + x_, 'y:' + this.handleY, 'top:' + this.area.top, 'bottom:' + this.area.bottom,'percent:' + this.percent, 'value:' + this.showValue)
				}
				else{
			...
			areaTouchStart(e) {
				if (!this.disabled){
					...
					if (this.direction == 'vertical'){
						// #ifdef H5
						tapX = this.area.height + e.changedTouches[0].clientY
						// #endif
						// #ifndef H5
						tapX = e.changedTouches[0].clientY
						// #endif
					}
					else{
						tapX = e.changedTouches[0].clientX - this.area.left - s[0] / 2 // 拖柄中心与鼠标箭头一致
					}
					tapX = tapX >= 0 ? tapX : 0
			...
			areaTouchMove(e) {
				if (!this.disabled){
					...
					if (this.direction == 'vertical'){
						...
						tapX = e.changedTouches[0].clientX - this.area.left - s[0] / 2
					}
					tapX = tapX >= 0 ? tapX : 0
					this.handleMove(tapX)
				}
