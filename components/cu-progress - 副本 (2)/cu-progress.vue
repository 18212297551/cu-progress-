<template>
	<view>
		<view class="cu-progress-main" :style="{'border-radius':bgBR,'width': progressMainW,'height':progressMainH,'background-color': backgroundColor}">
			...
			<text v-if="showInfoStatus || (this.showInfo && this.infoAlign == 'center' && this.direction != 'vertical')" class="cu-showInfo" :style="{'right':infoRt, 'left':infoLt, 'color':infoColor,'font-size':infoS,width:infoW}">{{showValue}}{{infoEndText}}</text>
		</view>
	</view>
</template>
